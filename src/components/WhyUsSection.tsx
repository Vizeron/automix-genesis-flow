...
const WhyUsSection = () => {
  ...
  const scenarios = {
    startups: {
      ...
      examples: ['Automated lead nurturing sequences that convert 40% better', 'Customer onboarding flows that reduce support tickets by 60%', 'Financial reporting that updates in real-time across all platforms']
    },
    agencies: {
      title: 'For Agencies',
      icon: Building,
      description: 'Deliver better results for clients while reducing operational overhead',
      examples: ['Client reporting dashboards that update automatically', 'Campaign optimization workflows that run 24/7', 'Resource allocation systems that maximize team efficiency']
    },
    ops: {
      title: 'For Ops Teams',
      icon: Users,
      description: 'Eliminate manual processes and focus on strategic initiatives',
      examples: ['Invoice processing that handles 99% of transactions automatically', 'Compliance monitoring that prevents issues before they occur', 'Data synchronization across all business systems']
    }
  };
  const tabs = Object.keys(scenarios) as Array<keyof typeof scenarios>;
  return <section className="py-20 bg-background-secondary/30">
      <div className="container mx-auto px-6">
        ...
        animationDelay: '0.2s'
          ...
          <div className="glass-card p-2 rounded-xl inline-flex space-x-2">
            ...
            return <button key={tab} onClick={() => setActiveTab(tab)} className={`flex items-center space-x-2 px-6 py-3 rounded-lg transition-all duration-300 ${isActive ? 'neuro-button-primary' : 'hover:bg-background-tertiary/50'}`}>
        ...
        animationDelay: '0.4s'
          ...
          <div className="glass-card p-8 lg:p-12 rounded-2xl">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {scenarios[activeTab as keyof typeof scenarios].examples.map((example, index) => <div key={index} className="glass-card p-6 rounded-xl border border-primary/20">
